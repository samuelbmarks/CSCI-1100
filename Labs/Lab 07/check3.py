def parse_line(line):
    ...
    split_list = line.rstrip().rsplit("/",3)
...
append_file = open("program.py", "w")

while fn != 0 and pn != 0 and ln != 0:
    pn = pn-1
    ln = ln-1    
    print(get_line(fn,pn,ln))
    results = parse_line(get_line(fn,pn,ln))
    if results[3] != "END":
        append_file.write(results[3]+"\n")
    fn = results[0]
    pn = results[1]
    ln = results[2]
